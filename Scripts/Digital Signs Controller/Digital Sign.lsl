    ...
    listen(integer channel, string name, key id, string msg)
    {
        ...
        if(llGetListLength(items)>1)
        {
            ...
            if(msgID==myID)
            {
                ...
                llSetPrimitiveParams([PRIM_TEXTURE, 3, textureUUID, <1.0,1.0,1.0>, <0.0,0.0,0.0>, 270*DEG_TO_RAD ]);
                llSetPrimitiveParams([PRIM_TEXTURE, 1, textureUUID, <1.0,1.0,1.0>, <0.0,0.0,0.0>, 270*DEG_TO_RAD ]);
            }
        }
    }
}